...
If you prefer to watch a video, an introduction is [available here](https://www.youtube.com/watch?v=Cxb1FUIsC04).
...
This is implemented as a LabVIEW class, but you do not need to know anything about object-oriented programming to use it.
...
Note that the ``Pass if Equal.vi`` assertion also works well for array data types, clusters and data classes.
...
The Setup VI will run once before each test method in the test case and the Teardown VI will run once after the test method is completed.
...
![Setup and Teardown execution order](img/setup_test_teardown.png)
...
The first run launches the UI, and the second is the actual test execution.
...
You can also launch the user interface from the tools menu through the ``Tools > LUnit > LUnit UI...`` menu option.
...
It makes sense to organize tests in some manner, especially as the number of tests increases.
...
All tests within the test suite project may then be executed using the right click menu option, but will not be executed when running all tests in the current project from the tools menu option and will not need to be loaded with the project.
...
In LUnit version 1.3.1, an additional feature was introduced to help speed up test execution by skipping slow or unrelated tests.
...
By toggling the `Ignore Dashed Tests` menu option, it is straightforward to optimize test execution for either execution speed or test coverage.
In a typical test driven development workflow, test execution speed must be very fast as the tests are executed repeatedly on a time scale of minutes.
