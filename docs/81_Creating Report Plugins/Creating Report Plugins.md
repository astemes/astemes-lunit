...
The only requirement for a reporting plugin is that it implements a specific interface and is placed in a specific folder within vi.lib.
...
To create a custom reporting plugin, you will need to create a class that implements the `LUnit Reporting Plugin.lvclass` interface.
...
One important limitation is that new references cannot be opened during `LUnit Result.vi` or they might be prematurely closed by the LabVIEW run-time.

It is important to understand the different types of results that are sent to `LUnit Result.vi`.
...
When you activate the plugin, you have the option to provide a configuration dialog.
...
The `.lvclass` file should be directly in the `plugins` directory, but the member VIs of the class will need to be placed in a sub directory to avoid naming conflicts.
...
If you open the plugin written in the old version, you will receive errors in LabVIEW.
Below is a step-by-step guide to resolve this.
...
2. Update the methods of the plugin to follow the new convention.
   ...
   These three methods should use the regular Dynamic Dispatch template connector pane with a Dynamic Dispatch Input and Output as well as the error terminals, and they need to be made reentrant.
   The `LUnit Start Test.vi` also needs a string array input where a list of tests is provided when the test is started.
